...
def setPrefix(listOfNames):
    ...
    for cur_name in listOfNames:
        if cur_name.startswith(pre) and pre != '':
            result = cmds.confirmDialog(title='Warning !', message='Prefix ' + str(
                pre) + ' already exists for ' + cur_name + ', continue?', button=['Yes', 'No'], defaultButton='Yes',
...
def setSuffix(listOfNames):
    ...
    for cur_name in listOfNames:
        if cur_name.endswith(suf) and suf != '':
            result = cmds.confirmDialog(title='Warning !', message='Suffix ' + str(
                suf) + ' already exists for ' + cur_name + ', continue?', button=['Yes', 'No'], defaultButton='Yes',
                                        cancelButton='No', dismissString='No')
            if result == 'Yes':
...
def renameToParent():
    checked = cmds.checkBox('RenameToParent', q=True, v=True)
    if checked:
        for name in setObjectType():
            parentName = cmds.listRelatives(name, parent=True)
            cmds.rename(name, parentName)
            countRename()
    global counter
    counter = 0

def autoRenameByClicking():
    cmds.scriptJob(e=['SelectionChanged', 'renameToParent()'], protected=True)
...
make_optmenu('optType', 'Rename by Type:',
             ['all', 'transform', 'mesh', 'joint', 'light', 'material', 'texture', 'camera'])
...
cmds.separator(height=5)
cmds.checkBox('RenameToParent', label='Auto rename to parent name', cc='autoRenameByClicking()')
